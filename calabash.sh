...
if [ -z $SAXON_JAR ]; then
    if [ -e $PROJECT_DIR/saxon/saxon12ee.jar ]; then
	SAXON_JAR=$PROJECT_DIR/saxon/saxon12ee.jar
    elif [ -e $PROJECT_DIR/saxon/saxon12pe.jar ]; then
        SAXON_JAR=$PROJECT_DIR/saxon/saxon12pe.jar
    else
...
DISTROLIBS="$DISTRO/lib/:$DISTRO/lib/xmlresolver-5.2.0.jar:$DISTRO/lib/xmlresolver-5.2.0-data.jar:$DISTRO/lib/commons-fileupload-1.4.jar:$DISTRO/lib/classindex-3.3.jar:$DISTRO/lib/htmlparser-1.4.jar:$PROJECT_DIR/a9s/common/calabash:$DISTRO/lib/org.restlet-2.2.2.jar:$MAIL_EXT:$DISTRO/lib/tagsoup-1.2.1.jar:$DIR/lib/slf4j-simple-1.7.36.jar:$DISTRO/lib/slf4j-api-1.7.36.jar"
